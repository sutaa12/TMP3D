/// <summary>
/// Text3D の内部処理を段階表示する説明用シェーダーです。
/// </summary>
/// <remarks>
/// SolidUnlit と同じ厚み付き Text3D 描画に加え、_GuideMode により compute ボックス、マスク座標、SDF、レイヒット、深度進行度などを可視化します。
/// </remarks>
Shader "NariEffect/Text3D/SolidUnlitExplainer"
{
    Properties
    {
        ...
        /// <summary>内部処理をどの段階として表示するかを選ぶモード値です。</summary>
        [Enum(Step01_ComputeBoxOnly,1,Step02_MaskCoordinates,2,Step03_AtlasSDFSlice,3,Step04_TextOnly,4,Step05_BoxRaycastHitTest,5,Step06_RaySteps,6,Step07_DepthProgress,7,Step08_FinalRendering,0)]
        _GuideMode("Explainer Mode", Float) = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            /// <summary>compute shader が出力した厚み付き文字頂点です。</summary>
            /// <remarks>BuildText3DGeometry.compute と Text3DSolidRenderer.SolidGlyphVertex と同じ順序・型を保ちます。</remarks>
            struct SolidGlyphVertex
            {
                /// <summary>ローカル空間の頂点位置です。</summary>
                float3 position;
                /// <summary>ローカル空間の面法線です。Explainer のボックス表示で色付けにも使います。</summary>
                float3 normal;
            ...
            /// <summary>頂点シェーダーからフラグメントシェーダーへ渡す Text3D 描画用データです。</summary>
            struct VertexToFragment
            {
                ...
                /// <summary>Explainer のボックス面色に使うローカル法線です。</summary>
                float3 localNormal     : TEXCOORD7;
            };

            /// <summary>色と深度を同時に返すフラグメント出力です。</summary>
            ...
            /// <summary>内部処理の可視化モードです。0 は最終描画、1..7 は段階表示です。</summary>
            float _GuideMode;
            ...
            // ====== 定数 ======
            #define MAX_STEPS 32
            #define GUIDE_FINAL 0
            #define GUIDE_BOX_ONLY 1
            #define GUIDE_MASK_COORDS 2
            #define GUIDE_ATLAS_SDF 3
            #define GUIDE_TEXT_ONLY 4
            #define GUIDE_RAYCAST_HIT_TEST 5
            #define GUIDE_RAY_STEPS 6
            #define GUIDE_DEPTH_PROGRESS 7

            /// <summary>値 x が a..b の範囲でどの位置にあるかを 0..1 基準で返します。</summary>
            float InverseLerp(float a, float b, float x){ return (x-a) / max(1e-6,(b-a)); }
            /// <summary>現在の _GuideMode が指定モードかを浮動小数誤差込みで判定します。</summary>
            bool GuideModeIs(float mode){ return abs(_GuideMode - mode) < 0.5; }
            ...
            /// <summary>SDF アトラス周辺 4 点からベベル用の疑似法線を作ります。</summary>
            ...
            /// <summary>法線とライト方向から Blinn-Phong 風のスペキュラー色を返します。</summary>
            fixed3 GetSpecular(fixed3 n, fixed3 l){
                half spec = pow(max(0, dot(n,l)), _Reflectivity);
                return _SpecularColor.rgb * spec * _SpecularPower;
            }

            /// <summary>SDF 距離に応じて表面色とアウトライン色をアルファ合成します。</summary>
            ...
            /// <summary>現在フラグメントのローカル空間視線方向、開始位置、現在位置、マスク座標です。</summary>
            ...
            /// <summary>レイが平面へ到達する距離を計算します。</summary>
            ...
            /// <summary>スラブ法でレイと AABB の交差区間を求めます。</summary>
            ...
            /// <summary>視線レイをローカル空間へ変換し、文字ボリュームとの交差範囲を初期化します。</summary>
            ...
            /// <summary>SDF 値から次に進む距離を決め、レイマーチを 1 ステップ進めます。</summary>
            ...
            /// <summary>SV_VertexID でインデックスバッファを引き、ソリッド頂点をクリップ空間へ変換します。</summary>
            ...
            /// <summary>デバッグキーワードが有効な場合に出力色をステップ数またはマスク座標へ差し替えます。</summary>
            ...
            /// <summary>現在のジオメトリ表面位置に色を付け、正しい深度で返す Explainer 用出力を作ります。</summary>
            FragmentOutput GuideSurfaceOutput(VertexToFragment i, fixed4 color)
            {
                FragmentOutput o;
                float3 lp = mul(_Text3DWorldToLocal, float4(i.worldPos.xyz, 1)).xyz;
            ...
            /// <summary>文字ごとに色を少し変えるための軽量ハッシュ値を返します。</summary>
            float GuideHash(float4 v)
            {
                return frac(sin(dot(v, float4(12.9898, 78.233, 37.719, 11.135))) * 43758.5453);
            }

            /// <summary>compute shader が作った箱形状と面向きを見やすい色で表示します。</summary>
            fixed4 GuideComputeBoxColor(VertexToFragment i)
            {
                float3 lp = mul(_Text3DWorldToLocal, float4(i.worldPos.xyz, 1)).xyz;
                float3 mask = saturate(PositionToMask(lp, i));
                float glyph = step(0.5, GuideHash(float4(i.boundariesLocal.xy, i.boundariesUV.xy)));

                float3 baseColor = lerp(float3(0.02, 0.42, 0.95), float3(0.0, 0.72, 1.0), glyph);
                baseColor += abs(normalize(i.localNormal + 1e-5)) * 0.12;

                float3 distToEdge = min(mask, 1.0 - mask);
                float3 edgeWidth = max(fwidth(mask) * 2.5, 0.015);
                float3 nearEdge = 1.0 - smoothstep(0.0, edgeWidth, distToEdge);
                float edgeLine = max(max(min(nearEdge.x, nearEdge.y), min(nearEdge.x, nearEdge.z)), min(nearEdge.y, nearEdge.z));

                float3 color = lerp(saturate(baseColor), float3(1, 1, 1), edgeLine);
                return fixed4(color, 1);
            }

            /// <summary>ローカル位置を 0..1 マスク座標に変換した結果を RGB として表示します。</summary>
            fixed4 GuideMaskColor(VertexToFragment i)
            {
                float3 lp = mul(_Text3DWorldToLocal, float4(i.worldPos.xyz, 1)).xyz;
                float3 mask = saturate(PositionToMask(lp, i));
                return fixed4(mask, 1);
            }

            /// <summary>現在位置の SDF アトラス値をグレースケールで表示します。</summary>
            fixed4 GuideAtlasSdfColor(VertexToFragment i)
            {
                float3 lp = mul(_Text3DWorldToLocal, float4(i.worldPos.xyz, 1)).xyz;
                float3 mask = saturate(PositionToMask(lp, i));
                float sdf = SampleSDF3D(mask, i);
                return fixed4(sdf, sdf, sdf, 1);
            }

            /// <summary>GuideMode に応じた可視化または通常の Text3D レイマーチ描画を行います。</summary>
            FragmentOutput Text3DUnlitFragment(VertexToFragment i)
            {
                ...
                if (GuideModeIs(GUIDE_BOX_ONLY))
                {
                    return GuideSurfaceOutput(i, GuideComputeBoxColor(i));
                }

                if (GuideModeIs(GUIDE_MASK_COORDS))
                {
                    return GuideSurfaceOutput(i, GuideMaskColor(i));
                }

                if (GuideModeIs(GUIDE_ATLAS_SDF))
                {
                    return GuideSurfaceOutput(i, GuideAtlasSdfColor(i));
                }
                ...
                for (int step=0; step<=MAX_STEPS; step++)
                {
                    ...
                    if (Temp_Bound >= 0 && Temp_Value <= edge)
                    {
                        ...
                        if (GuideModeIs(GUIDE_TEXT_ONLY))
                        {
                            float4 clipPos = mul(UNITY_MATRIX_VP, mul(_Text3DLocalToWorld, float4(lp,1)));
                            o.depth = ComputeDepth(clipPos);
                        ...
                        if (GuideModeIs(GUIDE_RAYCAST_HIT_TEST))
                        {
                            float4 clipPos = mul(UNITY_MATRIX_VP, mul(_Text3DLocalToWorld, float4(lp,1)));
                            o.depth = ComputeDepth(clipPos);
                        ...
                        if (GuideModeIs(GUIDE_RAY_STEPS))
                        {
                            float4 clipPos = mul(UNITY_MATRIX_VP, mul(_Text3DLocalToWorld, float4(lp,1)));
                            float stepTint = saturate((float)(step + 1) / (float)(MAX_STEPS + 1));
                            o.depth = ComputeDepth(clipPos);
                            o.color = fixed4(stepTint, stepTint, stepTint, 1);
                            return ValidateOutput(o, step);
                        }

                        if (GuideModeIs(GUIDE_DEPTH_PROGRESS))
                        {
                            float4 clipPos = mul(UNITY_MATRIX_VP, mul(_Text3DLocalToWorld, float4(lp,1)));
                            o.depth = ComputeDepth(clipPos);
                            o.color = fixed4(progress, progress, progress, 1);
                            return ValidateOutput(o, step);
                        }
                ...
                if (GuideModeIs(GUIDE_RAYCAST_HIT_TEST))
                {
                    return GuideSurfaceOutput(i, fixed4(1, 0, 0, 1));
                }
        ...
        Pass
        {
            ...
            struct SolidGlyphVertex {
                /// <summary>ローカル空間の頂点位置、法線、色、UV、厚み情報です。</summary>
            ...
            /// <summary>compute shader が生成したソリッド頂点・インデックスバッファです。</summary>
            ...
            half _Outline2Width;
            /// <summary>Explainer 表示中に Outline2 を抑止するためのモード値です。</summary>
            float _GuideMode;
            ...
            /// <summary>通常表示時だけ 2 段目アウトラインを短いレイマーチで描画します。</summary>
    ...
    CustomEditor "NariEffect.Text3D.Editor.Text3DExplainerMaterialInspector"
}
